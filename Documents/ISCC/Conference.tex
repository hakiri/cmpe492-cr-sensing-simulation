...
Figure \ref{fig:util} illustrates the utilization of PUs and SUs number of SUs in the cell for 1 zone and 36 zones cases. In both of these cases, SUs utilizes spectrum almost equally. However, in 1 zone case, SUs harms primary communication due to the high $\probm$ and $\mathbf{P_C}$ values.
...
We also compared our results with cooperation in whole cell case (1 zone case) and observed that applying cooperation with multiple zones results in better cooperation in terms of sensing reliability since with multiple zones cooperating in the cell, both miss detection and collision probabilities decrease.
