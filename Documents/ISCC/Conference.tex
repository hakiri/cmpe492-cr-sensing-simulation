...
These cases are illustrated in Figure \ref{fig:network}. At the left side of the cell secondary link is established over $f_2$ which is not used. Therefore, the $SINR$ value is high enough to have a communication. At the upper right side of the cell secondary link is established over $f_1$ which is used at other side of the cell which is far enough and at the different zone. Therefore, the $SINR$ value includes the interference but not so severe to degrade the communication link. At the lower right side of the cell secondary link uses $f_3$ which is used by primary user in close proximity. In this case collision occurs and the channel capacity is too low to have a valid communication. In each case the channel capacities are evaluated by the Shannon capacity formula.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
    C = B\,\, \log_2(1+SINR)
\end{equation}
...
As simulation output, we collected statistics about false alarm, missed detection, collision, blocking of SUs, dropping of SUs, and channel utilization. Note that in figures we do not plot all probability values for each zone. Instead we plot their mean against number of SUs.
