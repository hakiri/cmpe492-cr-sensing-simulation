...
public class DrawArea extends JPanel{
	...
	{
		super.paint(g);
		for(int i=0;i<RandomPositionDrawMain.rpdm.numberOfClusters;i++){
			PointColor pc = clusters.get(i);
			for(int j=0;j<RandomPositionDrawMain.rpdm.xij.get(i).size();j++){
				PointColor pn = nodes.get(RandomPositionDrawMain.rpdm.xij.get(i).get(j));
				g.setColor(Color.BLACK);
