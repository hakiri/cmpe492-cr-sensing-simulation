		...
		for(int i=0;i<xs.size();i++){
			for(int j=0;j<ys.get(i).size();j++){
				...
				for(int q=0;q<x.get(xs.get(i)).size();q++){
					pw.println(shapes[graphCounter%shapes.length]+" "+x.get(xs.get(i)).get(q)+" "+
	...
	private void legend(double xMin, double xMax, double yMin, double yMax, ArrayList<ArrayList<String>> names)
	{
		...
		for(int i=0;i<names.size();i++){
			for(int j=0;j<names.get(i).size();j++){
				pw.println(colors[graphCounter%colors.length]);
				pw.println(shapes[graphCounter%shapes.length]+" "+(xMax+xInc)+" "+(yMin+yInc*(2*graphCounter+1)));
				pw.println(shapes[graphCounter%shapes.length]+" "+(xMax+xInc*2)+" "+(yMin+yInc*(2*graphCounter+1)));
				pw.println(shapes[graphCounter%shapes.length]+" "+(xMax+xInc*3)+" "+(yMin+yInc*(2*graphCounter+1)));
